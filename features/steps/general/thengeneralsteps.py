...
@then(u'I should be in the "(?P<validation>.*)" "(?P<element_type>button|dropdown|option|a|div|h2|h1|span)"')
def step_impl(context, validation, element_type):
    element_name = transformation_helper(validation, element_type)
    elements = context.current_page.webElements.__dict__.get(element_name)
    return assert_that(context.current_page.validate_elements_present(elements), only_contains(True),
                       f'Some element is not present on the {validation} ')
