...
@when(u'I click on the "(?P<element_name>.*)" "(?P<element_type>button|dropdown|option|a|div|input)"')
...
@when(u'I click on the that "(?P<element_name>.*)" "(?P<element_type>button|dropdown|option|a|div|input)"')
def step_impl(context, element_name, element_type):
    element_name = transformation_helper(element_name, element_type)
    context.browser.find_element(context.current_page.webElements.__dict__.get(element_name)).click()
